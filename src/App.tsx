...
function App() {
  ...
  const { reservation, setReservation, createReservation, setError: setReservationError, error: reservationError } = useReservation()
  const { error: paymentError, confirmPayment, cancelPayment, created, setReservationId } = usePayment() 

  useEffect(() => {
    window.fetch("http://127.0.0.1:8080/health").then(body => body.json()).then(console.log).catch(console.error)
    ...
    eventSource.addEventListener('update', (event) => {
      try {
        ...
        console.log('serverSentEventsError:', error)
      }
    })
  ...
  const handleCancelPayment = async () => {
    const success = await cancelPayment({reservationId: reservation.id, reason: 'user cancelled payment'})
    if (success) {
      setReservation({})
  ...
  return (
    <div className="App">
      ...
      <div className={styles.body}>
        ...
        {reservation ? 
          <aside>You have one unpaid reservation <button onClick={handleSubmit}>Continue to Payment</button></aside>
        : <aside>
          <div className={styles.error}>{reservationError}</div>
          <div>
            <label>Name &nbsp;</label>
            <input type="text" placeholder="Enter name" onChange={evt => setName(evt.currentTarget.value)} value={name}/>
          </div>
          <br />

          <div>
            <label>Email &nbsp;</label>
            <input type="text" placeholder="Enter email" onChange={evt => setEmail(evt.currentTarget.value)} value={email}/>
          </div>
          <br />
